...
try {
    ...
    $pdf->SetTitle('Patient Records Export');
    $pdf->SetSubject('Patient Data');
    $pdf->SetKeywords('TCPDF, PDF, Patients, Export');
    ...
    // Title
    $pdf->Cell(0, 10, 'Patient Records Export', 0, 1, 'C');
    $pdf->Ln(5);
    ...
    if ($monthFilter) {
        ...
        $pdf->Cell(0, 10, 'Filtered by month: ' . $monthName, 0, 1, 'C');
    }
    ...
    $pdf->Output('patients_export_' . date('Y-m-d') . '.pdf', 'D');
